...
const Search = () => {
  ...
  const handleSubmit = (e: SubmitEvent<HTMLFormElement>) => {
    ...
    const sr = products.filter((product) =>
      JSON.stringify(product).toLowerCase().includes(query.trim().toLowerCase())
    );
    setSearchResults(sr);
  };
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Search for products on DoWell Shop" />
      </Helmet>
      <div class="mx-auto px-4 py-8 bg-slate-50">
        <form onSubmit={handleSubmit} class="mb-8 flex gap-2">
          <input
            class="flex h-10 border border-input text-sm w-full rounded-lg px-4 py-2 focus:border-2 focus:border-black outline-none"
